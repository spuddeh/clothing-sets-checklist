...
| --- | --- | --- | --- |
| `csc` | Clothing Sets Checklist | 26829 | main |
...
2. **File ID (manifest).** On the mod page, open the **Files** tab > **API Info** (or the Manage
   Files edit menu) to get the file ID, then replace the `REPLACE_WITH_CSC_FILE_ID` placeholder in
   `release-manifest.json` (the `file_id` field). Nexus still labels this value **"Group ID"** on the
   page, but it is what the upload action's `file_id` input wants. File IDs are not secret, so they
...
2. Create a GitHub Release whose **tag** follows `<artifact>-v<version>`:
   ```pwsh
   gh release create csc-v2.0.0 --title "Clothing Sets Checklist v2.0.0" --notes "..."
   ```
...
3. On publish, the workflow:
   ...
   - stages `contentDir` -> `installDir` and zips it as `<fileBaseName>_v<version>.zip` (e.g. `Clothing_Sets_Checklist_v2.0.0.zip`),
